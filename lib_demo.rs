...
struct Rewriter<'a> {
    ...
    group: Vec<&'a str>,
    component_map: &'a HashMap<String, String>,  // add this line
}

/// Traverses the given directory and returns a map from component names to their paths.
///
/// The paths are relative to the given directory and use forward slashes as separators.
fn create_component_map(dir: &Path) -> HashMap<String, String> {
    let mut map = HashMap::new();
    create_component_map_rec(dir, dir, &mut map, String::new());
    map
}

fn create_component_map_rec(dir: &Path, base_dir: &Path, map: &mut HashMap<String, String>, base_path: String) {
    for entry in fs::read_dir(dir).expect("Failed to read directory") {
        ...
        if metadata.is_dir() {
            let new_base_path = format!("{}/{}", base_path, path.file_name().unwrap().to_string_lossy());
            create_component_map_rec(&path, base_dir, map, new_base_path);
        } else if path.extension() == Some(std::ffi::OsStr::new("tsx")) {
            let component_name = path.file_stem().unwrap().to_string_lossy().into_owned();
            let relative_path = path.strip_prefix(base_dir).unwrap().to_string_lossy().into_owned();
            map.insert(component_name, relative_path.replace("\\", "/"));
        }
    }
}
        ...
        for spec in &old_decl.specifiers {
            match spec {
                ImportSpecifier::Named(named_spec) => {
                    ...
                    let import_name = old_decl.src.value.to_string();

                    let new_path = match self.component_map.get(&import_name) {
                        Some(mapped_path) => format!("@components/{}", mapped_path),
                        None => format!("@components/{}", import_name),
                    };
